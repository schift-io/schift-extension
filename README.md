...
```bash
npx -y @schift-io/extension install --host both
```
...
Credentials remain in `~/.schift/ai-memory/config.json`; the installer never
writes API keys into host configuration files. Remove the extension with
