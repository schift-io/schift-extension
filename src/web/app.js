...
const memory = document.querySelector("#graph-memory");
...
function renderGraph() {
  ...
  document.querySelector("#graph-purpose").textContent = purpose.value || "업무 목적";
  document.querySelector("#graph-model").textContent =
  ...
  memory.textContent = hasMemory ? "Schift Memory" : "Local only";
  tools.textContent = hasWrite
    ? "search · recall · approved write"
    : hasMemory
      ? "search · recall"
      : "no Schift data access";
}
